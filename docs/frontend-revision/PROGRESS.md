...
| PC-4 | "Never call `router.invalidate()`" for small mutations (§2.3) | **9 authenticated routes load outside the query cache**, so `invalidateQueries` is a no-op for them and scoped `router.invalidate({filter})` is the only mechanism that works. Includes Account 360, Quote detail, Job Sheet detail, Lead detail, Client detail. | Rule becomes: cache-loaded → narrow `crmQueryKeys`; loader-loaded → **scoped** `router.invalidate({filter})`, never bare; both when a mutation spans them. (VF-3) |
