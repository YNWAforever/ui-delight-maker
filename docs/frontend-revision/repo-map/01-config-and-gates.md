...
| `clientops:backfill-accounts` | `bun scripts/clientops/backfill-accounts.ts` | Reads `clients` where `account_id is null`, upserts `accounts`, links them (uses `src/server/db/neon.server` `query`/`queryOne`/`transaction`). Writes. | **Yes — DB (writes)** |
