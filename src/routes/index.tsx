...
import { moveLeadStage, triggerLeadAgent } from "@/server-functions/leads";
...
function PipelineCommandCenter() {
  ...
  useRealtime("leads", "*", undefined, () => router.invalidate());
  useRealtime("tasks", "*", undefined, () => router.invalidate());
  useRealtime("quotes", "*", undefined, () => router.invalidate());
  useRealtime("human_approvals", "*", undefined, () => router.invalidate());
  useRealtime("agent_runs", "*", undefined, () => router.invalidate());
  ...
  const draftReply = (lead: Lead) => {
    toast.message(`Reply drafting is not connected yet for ${lead.company_name}.`);
  };
