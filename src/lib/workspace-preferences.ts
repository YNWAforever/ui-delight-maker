...
export function normalizeWorkspaceViewConfig(value: unknown): WorkspaceViewConfig {
  ...
  if (typeof rawFilters.lifecycle_stage === "string" && lifecycleStages.has(rawFilters.lifecycle_stage as NonNullable<WorkspaceViewConfig["filters"]["lifecycle_stage"]>)) {
    filters.lifecycle_stage = rawFilters.lifecycle_stage as WorkspaceViewConfig["filters"]["lifecycle_stage"];
  }
  if (typeof rawFilters.account_owner === "string") filters.account_owner = rawFilters.account_owner;
  ...
  const visibleColumns = Array.isArray(value.columns)
    ? value.columns.filter((column): column is WorkspaceViewConfig["columns"][number] =>
        typeof column === "string" && columns.includes(column as (typeof columns)[number]),
      )
    : defaultConfig.columns;
  const rawSort = isRecord(value.sort) ? value.sort : {};
  const field = typeof rawSort.field === "string" && sortFields.includes(rawSort.field as (typeof sortFields)[number])
    ? (rawSort.field as WorkspaceViewConfig["sort"]["field"])
    : defaultConfig.sort.field;
  const direction = rawSort.direction === "asc" || rawSort.direction === "desc" ? rawSort.direction : defaultConfig.sort.direction;
