...
describe("route performance contract", () => {
  it("covers every user-facing route family", () => {
    expect(APP_ROUTE_FAMILIES.map((entry) => entry.id)).toEqual([
      "auth", "shell", "dashboard", "accounts", "clients", "leads", "campaigns",
      "quotes", "job-sheets", "tasks", "approvals", "renewals", "relationships",
      "notifications", "reports", "settings", "account", "agents", "ai-review", "admin",
    ]);
  });
  ...
  it("enforces payload, request, query, and chunk budgets", () => {
    ...
    expect(evaluateRouteMeasurement({
      routeFamily: "clients",
      fixture: "typical",
      serverCallCount: 1,
